...
REM extract objects within administrative boundary
pushd %UTIL%
REM osmfilter %INPUT% -v --keep="admin_level=%ADMIN_LEVEL% and name=Metro*Manila" --drop-nodes="place=" --drop-tags="*=" -o="%POLY%\%OUT_FILENAME%.osm"

osmfilter %INPUT% -v --parameter-file="%SCRIPTS%\1_extract_boundary\filter_boundary.txt"
